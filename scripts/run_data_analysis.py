...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run comprehensive BDD100K data analysis"
    )

    parser.add_argument(
        "--data-root", type=str, required=True, help="Root directory of BDD100K dataset"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
...
def run_data_parsing(data_root: str, output_dir: str, logger) -> Dict[str, Any]:
    ...
    if not all(validation.values()):
        logger.warning(
            "Dataset structure validation failed. Some files may be missing."
        )
...
def run_class_analysis(data: pd.DataFrame, output_dir: str, logger) -> Dict[str, Any]:
    ...
    Args:
        data: Combined annotation data from train and validation sets.
    ...
    logger.info(
        f"Class analysis completed. Generated {len(results['plot_files'])} plots"
    )
...
def run_spatial_analysis(
    data: pd.DataFrame, image_stats: Dict, output_dir: str, logger
) -> Dict[str, Any]:
    """
    Run comprehensive spatial analysis.

    Args:
        data: Combined annotation data with bounding box information.
        image_stats: Image statistics for context.
        output_dir: Directory to save analysis plots and reports.
        logger: Logger instance for status messages.
    ...
    logger.info(
        f"Spatial analysis completed. Generated {len(results['plot_files'])} plots"
    )
    logger.info(f"Analysis report: {results['report_file']}")
...
def run_image_analysis(
    ...
    Args:
        data: Combined annotation data.
        images_root: Path to directory containing images.
        output_dir: Directory to save analysis results.
        sample_images: Number of images to sample for dimension analysis.
        sample_quality: Number of images to sample for quality analysis.
        logger: Logger instance for status messages.
    ...
    results = analyzer.run_complete_analysis(
        dimension_sample=sample_images, quality_sample=sample_quality
    )

    logger.info(
        f"Image analysis completed. Generated {len(results['plot_files'])} plots"
    )
    logger.info(f"Analysis report: {results['report_file']}")
...
def generate_executive_summary(all_results: Dict[str, Any], output_dir: str, logger):
    ...
    Args:
        all_results: Dictionary containing results from all analysis steps.
        output_dir: Output directory for the summary file.
        logger: Logger instance for status messages.

    Returns:
        str: Path to the generated executive summary file.
    """
    ...
    summary_lines.extend(["## Data Quality Assessment", ""])
    ...
    if "parsing" in all_results:
        ...
        if total_errors == 0:
            summary_lines.append(
                "✅ **Excellent Data Quality**: No parsing errors or missing images detected"
            )
        elif total_errors < 100:
            summary_lines.append(
                f"⚠️ **Good Data Quality**: {total_errors} minor issues detected"
            )
        else:
            summary_lines.append(
                f"❌ **Data Quality Issues**: {total_errors} problems detected - review required"
            )
    ...
    if "class_analysis" in all_results:
        imbalance_ratio = all_results["class_analysis"]["summary"].get(
            "imbalance_ratio", 1
        )
        if imbalance_ratio > 10:
            summary_lines.append(
                "⚠️ **Address Class Imbalance**: Consider data augmentation, weighted sampling, or focal loss"
            )
        elif imbalance_ratio > 5:
            summary_lines.append(
                "ℹ️ **Monitor Class Balance**: Ensure validation metrics account for class distribution"
            )

    if "spatial_analysis" in all_results:
        summary_lines.append(
            "📍 **Spatial Patterns**: Review spatial distribution plots for class-specific positioning biases"
        )
    ...
    summary_lines.extend(["## Generated Analysis Files", ""])
    ...
    try:
        ...
        try:
            train_data = pd.read_csv(
                output_path / "processed" / "train_annotations.csv"
            )
        ...
        logger.info("\n" + "=" * 50)
        class_results = run_class_analysis(
            combined_data, str(output_path / "plots"), logger
        )
        ...
        if "parsing" in all_results and "image_validation" in all_results["parsing"]:
            image_stats = all_results["parsing"]["image_validation"].get(
                "image_statistics", {}
            )

        spatial_results = run_spatial_analysis(
            combined_data, image_stats, str(output_path / "plots"), logger
        )
        ...
        total_reports = sum(
            1
            for r in all_results.values()
            if isinstance(r, dict) and "report_file" in r
        )

        logger.info(
            f"📈 Generated {total_plots} plots and {total_reports} detailed reports"
        )
        logger.info(
            f"🚀 Start the dashboard: streamlit run src/visualization/dashboard.py"
        )
