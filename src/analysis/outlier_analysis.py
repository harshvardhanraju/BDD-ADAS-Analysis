...
class BDDOutlierAnalyzer:
    ...
    def __init__(
        ...
        Args:
            ...
            images_root: Root directory containing BDD100K images
        """
    ...
    def _load_data(self):
        ...
        try:
            self.train_data = pd.read_csv(
                self.data_dir / "analysis" / "processed" / "train_annotations.csv"
            )
            self.val_data = pd.read_csv(
                self.data_dir / "analysis" / "processed" / "val_annotations.csv"
            )
            ...
            with open(
                self.data_dir / "analysis" / "processed" / "parsing_statistics.json",
                "r",
            ) as f:
    ...
    def analyze_size_outliers(self, z_threshold: float = 3.0) -> Dict[str, Any]:
        ...
        Applies multiple outlier detection techniques including Z-score analysis,
        IQR method, extreme dimension ratios, and absolute size thresholds.

        Args:
            z_threshold: Z-score threshold for outlier detection (default: 3.0)

        Returns:
            Dictionary containing outlier counts, examples, and detailed analysis
        """
        print("🔍 Analyzing size outliers...")

        # Filter data with bounding boxes
        bbox_data = self.combined_data.dropna(
            subset=["bbox_area", "bbox_width", "bbox_height"]
        ).copy()
        ...
        bbox_data["dimension_ratio"] = (
            bbox_data["bbox_width"] / bbox_data["bbox_height"]
        )
        extreme_ratios = bbox_data[
            (bbox_data["dimension_ratio"] < 0.1) | (bbox_data["dimension_ratio"] > 10)
        ].copy()
        extreme_ratios["outlier_reason"] = "extreme_aspect_ratio"

        # 4. Unusually small objects (potential annotation errors)
        tiny_objects = bbox_data[
            bbox_data["bbox_area"] < 10
        ].copy()  # Less than 10 pixels
        tiny_objects["outlier_reason"] = "tiny_object"

        # 5. Unusually large objects (potential annotation errors)
        huge_objects = bbox_data[
            bbox_data["bbox_area"] > 100000
        ].copy()  # More than 100k pixels
        ...
        all_size_outliers = pd.concat(
            ...
            ignore_index=True,
        ).drop_duplicates(subset=["image_name", "bbox_x1", "bbox_y1"])
    ...
    def analyze_position_outliers(self) -> Dict[str, Any]:
        """
        Identify spatial and position outliers in object annotations.

        Detects objects near image edges, class-specific position anomalies,
        and invalid coordinate bounds.

        Returns:
            Dictionary containing position outlier analysis and examples
        """
        print("📍 Analyzing position outliers...")

        bbox_data = self.combined_data.dropna(
            subset=["bbox_x1", "bbox_y1", "bbox_x2", "bbox_y2"]
        ).copy()

        # Calculate normalized positions (assuming typical BDD100K dimensions)
        typical_width, typical_height = 1280, 720
        bbox_data["norm_center_x"] = (
            (bbox_data["bbox_x1"] + bbox_data["bbox_x2"]) / 2
        ) / typical_width
        bbox_data["norm_center_y"] = (
            (bbox_data["bbox_y1"] + bbox_data["bbox_y2"]) / 2
        ) / typical_height
        ...
        for class_name in bbox_data["category"].unique():
            ...
            x_mean, x_std = (
                class_data["norm_center_x"].mean(),
                class_data["norm_center_x"].std(),
            )
            y_mean, y_std = (
                class_data["norm_center_y"].mean(),
                class_data["norm_center_y"].std(),
            )
        ...
        all_position_outliers = (
            pd.concat(position_outliers, ignore_index=True)
            if position_outliers
            else pd.DataFrame()
        )
        if not all_position_outliers.empty:
            all_position_outliers = all_position_outliers.drop_duplicates(
                subset=["image_name", "bbox_x1", "bbox_y1"]
            )
        ...
        # Save examples
        if not all_position_outliers.empty:
            self._save_outlier_images(
                all_position_outliers, "position_outliers", max_samples=50
            )
    ...
    def analyze_annotation_outliers(self) -> Dict[str, Any]:
        """
        Identify annotation-related outliers and quality issues.

        Analyzes images with unusually high object counts, missing annotations,
        background-only images, and suspicious annotation patterns.

        Returns:
            Dictionary containing annotation outlier analysis and problematic cases
        """
        ...
        objects_per_image = (
            self.combined_data.groupby(["split", "image_name"])
            .agg({"category": "count", "bbox_area": "sum"})
        ...
        all_suspicious = (
            pd.concat(suspicious_patterns, ignore_index=True)
            if suspicious_patterns
            else pd.DataFrame()
        )
        if not all_suspicious.empty:
            all_suspicious = all_suspicious.drop_duplicates(
                subset=["image_name", "bbox_x1", "bbox_y1"]
            )
    ...
    def analyze_quality_outliers(self, sample_size: int = 1000) -> Dict[str, Any]:
        """
        Identify image quality outliers using computer vision metrics.

        Analyzes brightness, contrast, sharpness, and other quality indicators
        to identify corrupted or low-quality images.

        Args:
            sample_size: Number of images to analyze (default: 1000)

        Returns:
            Dictionary containing quality analysis and corrupted image examples
        """
                ...
                if image_path.exists():
                    ...
                    if img is None:
                        processing_errors.append(
                            {"image": image_name, "error": "Cannot read image"}
                        )
                        continue
                    ...
                    processing_errors.append(
                        {"image": image_name, "error": "File not found"}
                    )
        ...
        quality_outliers = {
            ...
            "corruption_breakdown": corrupted_images["corruption_reasons"]
            .value_counts()
            .to_dict(),
        ...
        if not corrupted_images.empty:
            self._save_outlier_images(
                ...
                max_samples=50,
                image_list=True,
                reason_col="corruption_reasons",
            )

        return quality_outliers

    def _save_outlier_images(
        self,
        outlier_data: pd.DataFrame,
        outlier_type: str,
        max_samples: int = 50,
        image_list: bool = False,
        reason_col: str = "outlier_reason",
    ):
        """
        Save outlier images to inspection folder for manual review.

        Args:
            outlier_data: DataFrame containing outlier information
            outlier_type: Type of outliers (used for folder naming)
            max_samples: Maximum number of samples to save
            image_list: Whether data contains image info directly
            reason_col: Column name containing outlier reason
        """
        if outlier_data.empty:
            return

        print(
            f"  💾 Saving {min(len(outlier_data), max_samples)} {outlier_type} examples..."
        )
        ...
        for idx, row in samples.iterrows():
            try:
                ...
                if source_path.exists():
                    ...
                    safe_reason = "".join(
                        c for c in str(reason) if c.isalnum() or c in "._-"
                    )
                    dest_filename = (
                        f"{split}_{image_name.split('.')[0]}_{safe_reason}.jpg"
                    )
    ...
    def _save_missing_annotation_images(self, missing_images: List[str]):
        """
        Save images that are missing annotations for manual inspection.

        Args:
            missing_images: List of image names without annotations
        """
    ...
    def generate_outlier_summary_report(self, results: Dict[str, Any]) -> str:
        """
        Generate comprehensive outlier analysis report in Markdown format.

        Args:
            results: Dictionary containing all outlier analysis results
            ...
            if corruption_breakdown:
                report_lines.extend(
                    [
                        ...
                        *[
                            f"- {issue}: {count}"
                            for issue, count in corruption_breakdown.items()
                        ],
                        "",
                    ]
                )
    ...
    def run_complete_outlier_analysis(self) -> Dict[str, Any]:
        """
        Run comprehensive outlier analysis covering all aspects.

        Executes size, position, annotation, and quality outlier detection,
        saves example images, and generates summary report.

        Returns:
            Dictionary containing all analysis results and report file path
        """
    ...
    for analysis_type, data in results.items():
        if isinstance(data, dict) and "error" not in data:
            if analysis_type == "size_outliers":
                ...
                print(
                    f"  Position Outliers: {data.get('total_position_outliers', 0):,}"
                )
            elif analysis_type == "annotation_outliers":
                print(
                    f"  Missing Annotations: {data.get('missing_annotation_images', 0):,}"
                )
