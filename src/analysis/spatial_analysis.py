...
class SpatialAnalyzer:
    ...
    def __init__(self, data: pd.DataFrame, image_stats: Optional[Dict] = None, 
        ...
        self.bbox_data['bbox_center_x'] = (self.bbox_data['bbox_x1'] + self.bbox_data['bbox_x2']) / 2
        self.bbox_data['bbox_center_y'] = (self.bbox_data['bbox_y1'] + self.bbox_data['bbox_y2']) / 2
    ...
    def analyze_bbox_dimensions(self) -> Dict[str, Any]:
        ...
        size_distribution = self.bbox_data.groupby(['category', 'size_category']).size().unstack(fill_value=0)
    ...
    def analyze_spatial_distribution(self) -> Dict[str, Any]:
        ...
        data_for_grid['grid_x'] = pd.cut(data_for_grid[x_col], bins=x_bins, labels=['Left', 'Center', 'Right'])
        data_for_grid['grid_y'] = pd.cut(data_for_grid[y_col], bins=y_bins, labels=['Top', 'Middle', 'Bottom'])
        data_for_grid['grid_cell'] = data_for_grid['grid_y'].astype(str) + '_' + data_for_grid['grid_x'].astype(str)
    ...
    def analyze_class_spatial_patterns(self) -> Dict[str, Any]:
        """Analyze spatial patterns specific to each class."""
        class_patterns = {}
        ...
        for class_name in self.classes:
            ...
            if self.normalized_data is not None:
                class_norm = self.normalized_data[self.normalized_data['category'] == class_name]
            ...
            dominant_position = max(vertical_distribution.items(), key=lambda x: x[1])[0]
            ...
            class_patterns[class_name] = {
                ...
                'spatial_concentration': 1 / (center_x_std * center_y_std) if center_x_std > 0 and center_y_std > 0 else 0
            }
    ...
    def detect_spatial_anomalies(self) -> Dict[str, Any]:
        ...
        anomaly_analysis['size_anomalies'] = {
            ...
            'examples': size_anomalies[['image_name', 'category', 'bbox_area']].head(10).to_dict('records')
        }
        ...
        anomaly_analysis['aspect_ratio_anomalies'] = {
            ...
            'examples': ar_anomalies[['image_name', 'category', 'bbox_aspect_ratio']].head(10).to_dict('records')
        }
        ...
        if self.normalized_data is not None:
            ...
            anomaly_analysis['edge_anomalies'] = {
                ...
                'examples': edge_anomalies[['image_name', 'category']].head(10).to_dict('records')
            }
    ...
    def create_spatial_plots(self) -> List[str]:
        ...
        # Width distribution
        self.bbox_data['bbox_width'].hist(bins=50, ax=axes[0,0], alpha=0.7, color='skyblue')
        axes[0,0].set_title('Bounding Box Width Distribution', fontweight='bold')
        axes[0,0].set_xlabel('Width (pixels)')
        axes[0,0].set_ylabel('Frequency')
        axes[0,0].axvline(self.bbox_data['bbox_width'].mean(), color='red', linestyle='--', label='Mean')
        axes[0,0].legend()
        
        # Height distribution
        self.bbox_data['bbox_height'].hist(bins=50, ax=axes[0,1], alpha=0.7, color='lightcoral')
        axes[0,1].set_title('Bounding Box Height Distribution', fontweight='bold')
        axes[0,1].set_xlabel('Height (pixels)')
        axes[0,1].set_ylabel('Frequency')
        axes[0,1].axvline(self.bbox_data['bbox_height'].mean(), color='red', linestyle='--', label='Mean')
        axes[0,1].legend()
        
        # Area distribution (log scale)
        axes[0,2].hist(self.bbox_data['bbox_area'], bins=50, alpha=0.7, color='lightgreen')
        axes[0,2].set_title('Bounding Box Area Distribution', fontweight='bold')
        axes[0,2].set_xlabel('Area (pixels²)')
        axes[0,2].set_ylabel('Frequency')
        axes[0,2].set_yscale('log')
        axes[0,2].axvline(self.bbox_data['bbox_area'].mean(), color='red', linestyle='--', label='Mean')
        axes[0,2].legend()
        
        # Aspect ratio distribution
        self.bbox_data['bbox_aspect_ratio'].hist(bins=50, ax=axes[1,0], alpha=0.7, color='orange')
        axes[1,0].set_title('Aspect Ratio Distribution', fontweight='bold')
        axes[1,0].set_xlabel('Aspect Ratio (Width/Height)')
        axes[1,0].set_ylabel('Frequency')
        axes[1,0].axvline(1.0, color='black', linestyle='--', alpha=0.5, label='Square (1:1)')
        axes[1,0].axvline(self.bbox_data['bbox_aspect_ratio'].mean(), color='red', linestyle='--', label='Mean')
        axes[1,0].legend()
        ...
        scatter = axes[1,1].scatter(sample_data['bbox_width'], sample_data['bbox_height'], 
                                  c=sample_data.index, alpha=0.6, cmap='viridis', s=1)
        ...
        im1 = axes[0,0].imshow(hist.T, origin='lower', extent=[0, 1, 0, 1], 
        ...
        if 'spatial_distribution' in self.analysis_results:
            grid_dist = self.analysis_results['spatial_distribution']['grid_distribution']
            ...
            for i in range(3):
                for j in range(3):
                    axes[0,1].text(j, i, f'{int(grid_matrix[i, j]):,}', 
        ...
        for idx, class_name in enumerate(top_classes):
            ...
            ax_idx = (1, idx)
            im = axes[ax_idx].imshow(hist_class.T, origin='lower', extent=[0, 1, 0, 1], 
                                   cmap='YlOrRd', aspect='equal')
            axes[ax_idx].set_title(f'{class_name.title()} Spatial Distribution', fontweight='bold')
        ...
        class_sample = self.bbox_data.sample(n=min(5000, len(self.bbox_data)))  # Sample for performance
        ...
        axes[1,0].set_title('Area Distribution by Class (Log Scale)', fontweight='bold')
        axes[1,0].set_yscale('log')
        axes[1,0].tick_params(axis='x', rotation=45)
        
        # Aspect ratio by class
        sns.boxplot(data=class_sample, x='category', y='bbox_aspect_ratio', ax=axes[1,1])
        axes[1,1].set_title('Aspect Ratio Distribution by Class', fontweight='bold')
        axes[1,1].tick_params(axis='x', rotation=45)
        axes[1,1].axhline(y=1.0, color='red', linestyle='--', alpha=0.5, label='Square (1:1)')
        axes[1,1].legend()
    ...
    def generate_spatial_report(self) -> str:
        ...
        if 'bbox_dimensions' in self.analysis_results:
            ...
            sorted_classes = sorted(class_stats.items(), 
                                  key=lambda x: x[1]['avg_area'], reverse=True)
            
            for i, (class_name, stats) in enumerate(sorted_classes[:5]):
                report_lines.append(f"{i+1}. {class_name}: {stats['avg_area']:.0f} pixels² "
        ...
        if 'spatial_distribution' in self.analysis_results:
            ...
            if 'grid_distribution' in spatial_stats:
                ...
                for position, count in sorted(grid_dist.items(), key=lambda x: x[1], reverse=True):
                    percentage = count / total_objects * 100
                    report_lines.append(f"- {position.replace('_', ' ')}: {count:,} ({percentage:.1f}%)")
                report_lines.append("")
            
            if 'clustering' in spatial_stats:
                clustering = spatial_stats['clustering']
                report_lines.append("### Spatial Clustering Analysis")
                report_lines.append(f"- Number of spatial clusters: {clustering['n_clusters']}")
        ...
        if 'class_spatial_patterns' in self.analysis_results:
            ...
            for class_name, pattern in patterns.items():
                ...
                dominant_pos = pattern['dominant_vertical_position'].replace('_', ' ').replace(' ratio', '')
                ...
                report_lines.append(f"- Spatial concentration score: {pattern['spatial_concentration']:.3f}")
                
                report_lines.append("- Vertical distribution:")
                report_lines.append(f"  - Top third: {vertical_dist['top_third_ratio']:.1%}")
                report_lines.append(f"  - Middle third: {vertical_dist['middle_third_ratio']:.1%}")
                report_lines.append(f"  - Bottom third: {vertical_dist['bottom_third_ratio']:.1%}")
                report_lines.append("")
            ...
            for anomaly_type, info in anomalies.items():
                ...
                if 'examples' in info and info['examples']:
                    ...
                    for example in info['examples'][:3]:
                        report_lines.append(f"  - {example['image_name']} ({example['category']})")
                report_lines.append("")
    ...
    def run_complete_analysis(self) -> Dict[str, Any]:
        """Run all spatial analysis components."""
