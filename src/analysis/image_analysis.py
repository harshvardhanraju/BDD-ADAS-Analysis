...
class ImageCharacteristicsAnalyzer:
    ...
    def analyze_image_dimensions(
        ...
        Returns:
            Dictionary containing dimension statistics and format distributions
        """
        dimension_analysis = {}
        ...
        if sample_size and len(unique_images) > sample_size:
            ...
            print(
                f"Sampling {sample_size} images from {len(unique_images)} total images"
            )
        else:
            ...
            try:
                if image_path.exists():
                    ...
                    processing_errors.append(
                        {"image": f"{split}/{image_name}", "error": "File not found"}
                    )

            except Exception as e:
                processing_errors.append(
                    {"image": f"{split}/{image_name}", "error": str(e)}
                )
        ...
        resolution_counts = (
            df_dims.groupby(["width", "height"]).size().reset_index(name="count")
        )
        resolution_counts["resolution"] = (
            resolution_counts["width"].astype(str)
            + "x"
            + resolution_counts["height"].astype(str)
        )
        top_resolutions = resolution_counts.sort_values("count", ascending=False).head(
            10
        )
        ...
        for split in self.splits:
            ...
            if len(split_data) > 0:
                split_stats[split] = {
                    ...
                    "resolution_consistency": len(
                        split_data.groupby(["width", "height"])
                    )
                    == 1,
                }
    ...
    def analyze_scene_attributes(self) -> Dict[str, Any]:
        ...
        Extracts and analyzes image attributes like weather conditions,
        time of day, scene type, and other metadata.

        Returns:
            Dictionary containing attribute statistics and correlations
        """
        scene_analysis = {}
        ...
        attribute_columns = [
            col for col in images_with_attributes.columns if col.startswith("img_attr_")
        ]
        ...
        for split in self.splits:
            split_data = images_with_attributes[
                images_with_attributes["split"] == split
            ]
            split_stats = {}
    ...
    def analyze_temporal_patterns(self) -> Dict[str, Any]:
        ...
        Examines timestamp information to understand temporal distribution
        including hourly patterns, day-of-week trends, and video sequences.

        Returns:
            Dictionary containing temporal analysis and video statistics
        """
        temporal_analysis = {}
        ...
        temporal_data = self.annotation_data[
            ["split", "image_name", "timestamp", "video_name"]
        ].drop_duplicates()
        ...
        temporal_data["datetime"] = pd.to_datetime(
            temporal_data["timestamp"], unit="ms", errors="coerce"
        )
        ...
        if "video_name" in temporal_data.columns:
            video_stats = temporal_data.groupby("video_name").agg(
                {"timestamp": ["count", "min", "max"], "image_name": "nunique"}
            )
            ...
            video_stats["duration_ms"] = (
                video_stats["end_timestamp"] - video_stats["start_timestamp"]
            )
        ...
        temporal_analysis = {
            ...
            "temporal_range": {
                ...
                "total_timespan_days": (
                    temporal_data["datetime"].max() - temporal_data["datetime"].min()
                ).days,
            },
    ...
    def analyze_image_quality(self, sample_size: Optional[int] = 500) -> Dict[str, Any]:
        """
        Analyze image quality metrics including brightness, contrast, and sharpness.

        Uses computer vision techniques to assess image quality and identify
        potential issues that could affect model training.

        Args:
            sample_size: Number of images to sample for quality analysis

        Returns:
            Dictionary containing quality metrics and outlier analysis
        """
        quality_analysis = {}
        ...
        for metric in quality_metrics:
            ...
            outliers[metric] = {
                ...
                "examples": outlier_images[["image_name", "split", metric]]
                .head(5)
                .to_dict("records"),
            }
        ...
        quality_analysis = {
            ...
            "processing_summary": {
                ...
                "total_errors": len(processing_errors),
                "error_rate": len(processing_errors)
                / (len(df_quality) + len(processing_errors))
                * 100,
            },
        }
    ...
    def create_image_analysis_plots(self) -> List[str]:
        """
        Create comprehensive image analysis visualization plots.

        Generates plots for dimensions, scene attributes, temporal patterns,
        and quality metrics based on analysis results.
        ...
        if "image_dimensions" in self.analysis_results:
            ...
            if (
                "overall_statistics" in dims
                and "aspect_ratio" in dims["overall_statistics"]
            ):
                ...
                axes[1, 0].axvline(
                    ar_mean, color="red", linestyle="--", label=f"Mean: {ar_mean:.2f}"
                )
                axes[1, 0].legend()
        ...
        if "scene_attributes" in self.analysis_results:
            scene_attrs = self.analysis_results["scene_attributes"][
                ...
                for i, (attr_name, attr_stats) in enumerate(scene_attrs.items()):
                    if i < len(axes):
                        ...
                        if len(labels) <= 10:  # Bar chart for few categories
                            ...
                            axes[i].set_title(
                                f"{attr_name.title()} Distribution", fontweight="bold"
                            )
                            ...
                            axes[i].pie(
                                top_values, labels=top_labels, autopct="%1.1f%%"
                            )
                            axes[i].set_title(
                                f"{attr_name.title()} Distribution (Top 10)",
                                fontweight="bold",
                            )
        ...
        if "temporal_patterns" in self.analysis_results:
            ...
            if "monthly_distribution" in temporal:
                ...
                axes[1, 0].bar(
                    [month_names[m - 1] for m in months], counts, color="lightgreen"
                )
            ...
            for i, metric in enumerate(quality_metrics):
                if i < 6:  # Only show first 6 metrics
                    ...
                    axes[row, col].set_title(
                        f"{metric.title()} Distribution", fontweight="bold"
                    )
    ...
    def generate_image_analysis_report(self) -> str:
        """
        Generate comprehensive image analysis report in text format.

        Creates detailed report covering all aspects of image characteristics
        ...
        if "image_dimensions" in self.analysis_results:
            ...
            if "overall_statistics" in dims:
                ...
                for dimension in ["width", "height"]:
                    s = stats[dimension]
                    report_lines.append(
                        f"- {dimension.title()}: {s['mean']:.0f} ± {s['std']:.0f} pixels"
                    )
                    report_lines.append(
                        f"  Range: {s['min']:.0f} - {s['max']:.0f} pixels"
                    )
                    report_lines.append(f"  Unique values: {s['unique_values']}")

                ar_stats = stats["aspect_ratio"]
                report_lines.append(
                    f"- Aspect Ratio: {ar_stats['mean']:.2f} ± {ar_stats['std']:.2f}"
                )
                report_lines.append("")
            ...
            if "top_resolutions" in dims:
                ...
                for i, res in enumerate(dims["top_resolutions"][:5]):
                    ...
                    report_lines.append(
                        f"{i+1}. {res['width']}×{res['height']}: {res['count']:,} images ({percentage:.1f}%)"
                    )
                report_lines.append("")
            ...
            if "format_distribution" in dims:
                ...
                for format_type, count in dims["format_distribution"].items():
                    ...
                    report_lines.append(
                        f"- {format_type}: {count:,} images ({percentage:.1f}%)"
                    )
                report_lines.append("")
        ...
        if "scene_attributes" in self.analysis_results:
            ...
            if "attribute_statistics" in scene:
                for attr_name, attr_stats in scene["attribute_statistics"].items():
                    ...
                    report_lines.append(
                        f"- Total images with this attribute: {attr_stats['total_images']:,}"
                    )
                    report_lines.append(
                        f"- Unique values: {attr_stats['unique_values']}"
                    )
                    report_lines.append(
                        f"- Most common value: {attr_stats['most_common']}"
                    )

                    # Show top 3 distributions
                    sorted_dist = sorted(
                        attr_stats["distribution"].items(),
                        key=lambda x: x[1],
                        reverse=True,
                    )
        ...
        if "temporal_patterns" in self.analysis_results:
            ...
            if "temporal_range" in temporal:
                ...
                report_lines.append(
                    f"- Dataset temporal range: {range_info['earliest']} to {range_info['latest']}"
                )
                report_lines.append(
                    f"- Total timespan: {range_info['total_timespan_days']} days"
                )
                report_lines.append("")
            ...
            if "hourly_distribution" in temporal:
                ...
                report_lines.append(
                    f"- Peak hour: {peak_hour}:00 ({peak_count:,} images)"
                )
                ...
                report_lines.append(
                    f"- Quietest hours: {', '.join([f'{h}:00' for h, c in quiet_hours])}"
                )
                report_lines.append("")
            ...
            if "video_analysis" in temporal and temporal["video_analysis"]:
                ...
                report_lines.append(
                    f"- Total videos: {video_stats.get('total_videos', 0):,}"
                )
                report_lines.append(
                    f"- Average video duration: {video_stats.get('avg_video_duration', 0):.1f} seconds"
                )
                report_lines.append(
                    f"- Average frames per video: {video_stats.get('avg_frames_per_video', 0):.1f}"
                )
                report_lines.append(
                    f"- Total content duration: {video_stats.get('total_duration_hours', 0):.1f} hours"
                )
                report_lines.append("")
            ...
            if "outlier_analysis" in quality:
                ...
                for metric, outlier_info in quality["outlier_analysis"].items():
                    if outlier_info["count"] > 0:
                        report_lines.append(
                            f"- {metric.title()}: {outlier_info['count']:,} outliers ({outlier_info['percentage']:.1f}%)"
                        )
                report_lines.append("")
        ...
        for analysis_type, results in self.analysis_results.items():
            if "total_processed" in results:
                report_lines.append(
                    f"- {analysis_type.replace('_', ' ').title()}: {results['total_processed']:,} images processed"
                )

                if "total_errors" in results:
                    error_rate = (
                        results["total_errors"]
                        / (results["total_processed"] + results["total_errors"])
                        * 100
                    )
                    report_lines.append(
                        f"  - Errors: {results['total_errors']:,} ({error_rate:.1f}%)"
                    )
    ...
    def run_complete_analysis(
        self, dimension_sample: int = 1000, quality_sample: int = 500
    ) -> Dict[str, Any]:
        """
        Run all image analysis components in sequence.

        Args:
            dimension_sample: Sample size for dimension analysis
            quality_sample: Sample size for quality analysis
        ...
        return {
            ...
            "summary": {
                "total_unique_images": len(
                    self.annotation_data[["split", "image_name"]].drop_duplicates()
                ),
