...
class ClassDistributionAnalyzer:
    ...
    def __init__(self, data: pd.DataFrame, output_dir: str = "data/analysis/plots"):
        ...
        self.object_data = data[data['category'].notna() & (data['category'] != '')].copy()
    ...
    def compute_basic_statistics(self) -> Dict[str, Any]:
        ...
        for split in self.splits:
            ...
            split_stats[split] = {
                ...
                'avg_objects_per_image': len(split_data) / split_data['image_name'].nunique()
            }
    ...
    def analyze_split_consistency(self) -> Dict[str, Any]:
        """Analyze consistency of class distribution across splits."""
        ...
        cv_values = {k: v['coefficient_of_variation'] for k, v in cv_analysis.items() 
                    if not np.isinf(v['coefficient_of_variation'])}
        
        if cv_values:
            consistency_analysis['most_consistent_class'] = min(cv_values, key=cv_values.get)
            consistency_analysis['least_consistent_class'] = max(cv_values, key=cv_values.get)
        
        self.analysis_results['split_consistency'] = consistency_analysis
        return consistency_analysis
    
    def analyze_co_occurrence(self) -> Dict[str, Any]:
        """Analyze co-occurrence patterns between classes."""
        # Group by image to find co-occurring classes
        image_classes = self.object_data.groupby(['split', 'image_name'])['category'].apply(list)
        ...
        conditional_prob_matrix = co_occurrence_matrix.div(class_counts, axis=0).fillna(0)
        
        # Find most frequent pairs
        top_pairs = sorted(co_occurrence_counts.items(), key=lambda x: x[1], reverse=True)[:10]
        ...
        for class_name in self.classes:
            ...
            isolation_score = 1 - (co_occurrence_sum / total_occurrences) if total_occurrences > 0 else 0
            isolation_scores[class_name] = isolation_score
        
        co_occurrence_analysis['isolation_scores'] = isolation_scores
        co_occurrence_analysis['most_isolated_class'] = max(isolation_scores, key=isolation_scores.get)
        co_occurrence_analysis['least_isolated_class'] = min(isolation_scores, key=isolation_scores.get)
    ...
    def create_distribution_plots(self) -> List[str]:
        ...
        # Linear scale
        overall_counts = self.object_data['category'].value_counts()
        bars1 = ax1.bar(range(len(overall_counts)), overall_counts.values, 
                       color=sns.color_palette("husl", len(overall_counts)))
        ax1.set_xlabel('Object Classes')
        ax1.set_ylabel('Count (Linear Scale)')
        ax1.set_title('Overall Class Distribution - Linear Scale', fontsize=14, fontweight='bold')
        ...
        # Log scale
        bars2 = ax2.bar(range(len(overall_counts)), overall_counts.values, 
                       color=sns.color_palette("husl", len(overall_counts)))
        ax2.set_xlabel('Object Classes')
        ax2.set_ylabel('Count (Log Scale)')
        ax2.set_title('Overall Class Distribution - Log Scale', fontsize=14, fontweight='bold')
        ax2.set_yscale('log')
        ...
        # Stacked bar chart
        split_crosstab = pd.crosstab(self.object_data['category'], self.object_data['split'])
        split_crosstab.plot(kind='bar', stacked=True, ax=axes[0,0], 
                           color=['#1f77b4', '#ff7f0e'])
        ...
        axes[0,1].set_title('Class Distribution by Split (Side-by-side)', fontweight='bold')
        ...
        axes[1,0].set_title('Class Distribution Proportions by Split', fontweight='bold')
        ...
        for class_name in split_crosstab.index:
            if len(self.splits) >= 2:
                ratio = split_crosstab.loc[class_name].max() / split_crosstab.loc[class_name].min()
                imbalance_ratios.append(ratio)
                class_names.append(class_name)
        
        axes[1,1].bar(range(len(imbalance_ratios)), imbalance_ratios, 
                     color='red', alpha=0.7)
        ...
        if hasattr(self, 'analysis_results') and 'co_occurrence' in self.analysis_results:
            co_occ_matrix = pd.DataFrame(self.analysis_results['co_occurrence']['conditional_probability_matrix'])
            
            plt.figure(figsize=(12, 10))
            sns.heatmap(co_occ_matrix, annot=True, cmap='YlOrRd', fmt='.3f',
                       square=True, cbar_kws={'label': 'Conditional Probability'})
            plt.title('Class Co-occurrence Conditional Probability Matrix', 
    ...
    def create_statistical_summary_plot(self) -> str:
        ...
        for i, split in enumerate(self.splits):
            split_objects = objects_per_image[objects_per_image.index.get_level_values(0) == split]
            axes[0,1].hist(split_objects.values, bins=30, alpha=0.7, 
        ...
        overall_counts = self.object_data['category'].value_counts()
        axes[0,2].hist(overall_counts.values, bins=20, color='skyblue', alpha=0.7, edgecolor='black')
        ...
        axes[0,2].axvline(overall_counts.mean(), color='red', linestyle='--', 
                         label=f'Mean: {overall_counts.mean():.0f}')
        axes[0,2].axvline(overall_counts.median(), color='orange', linestyle='--', 
        ...
        axes[1,0].axhline(y=80, color='red', linestyle='--', alpha=0.7, label='80% Coverage')
        axes[1,0].axhline(y=90, color='orange', linestyle='--', alpha=0.7, label='90% Coverage')
        axes[1,0].legend()
        ...
        axes[1,2].bar(x_pos - width/2, top_5.values, width, label='Top 5 Classes', 
                     color='darkgreen', alpha=0.7)
        axes[1,2].bar(x_pos + width/2, bottom_5.values, width, label='Bottom 5 Classes', 
    ...
    def generate_analysis_report(self) -> str:
        ...
        if 'basic_statistics' in self.analysis_results:
            ...
            for i, (class_name, count) in enumerate(list(stats['overall_distribution'].items())[:10]):
                percentage = count / stats['total_objects'] * 100
                report_lines.append(f"{i+1:2d}. {class_name:<15}: {count:>8,} ({percentage:5.1f}%)")
            report_lines.append("")
            
            # Imbalance metrics
            imb = stats['imbalance_metrics']
            report_lines.append("### Imbalance Analysis")
            report_lines.append(f"- Imbalance Ratio (max/min): {imb['imbalance_ratio']:.2f}")
            report_lines.append(f"- Gini Coefficient: {imb['gini_coefficient']:.3f} (0=perfect equality, 1=perfect inequality)")
            report_lines.append(f"- Normalized Entropy: {imb['normalized_entropy']:.3f} (1=perfect balance, 0=perfect imbalance)")
            report_lines.append(f"- Most frequent class: {imb['most_frequent_class']} ({imb['most_frequent_count']:,} objects)")
            report_lines.append(f"- Least frequent class: {imb['least_frequent_class']} ({imb['least_frequent_count']:,} objects)")
            report_lines.append("")
            ...
            for split, split_stats in stats['split_statistics'].items():
                ...
                report_lines.append(f"- Total objects: {split_stats['total_objects']:,}")
                report_lines.append(f"- Images with objects: {split_stats['images_with_objects']:,}")
                report_lines.append(f"- Average objects per image: {split_stats['avg_objects_per_image']:.1f}")
                report_lines.append("")
        ...
        if 'split_consistency' in self.analysis_results:
            ...
            report_lines.append(f"- Chi-square test p-value: {chi2_test['p_value']:.6f}")
            report_lines.append(f"- Splits are {'consistent' if chi2_test['is_consistent'] else 'inconsistent'} (α=0.05)")
            
            if 'most_consistent_class' in consistency:
                report_lines.append(f"- Most consistent class across splits: {consistency['most_consistent_class']}")
                report_lines.append(f"- Least consistent class across splits: {consistency['least_consistent_class']}")
            report_lines.append("")
        ...
        if 'co_occurrence' in self.analysis_results:
            ...
            for i, pair_info in enumerate(co_occ['top_co_occurring_pairs'][:5]):
                ...
                report_lines.append(f"{i+1}. {classes[0]} & {classes[1]}: {count:,} images ({percentage:.1f}%)")
            
            report_lines.append("")
            report_lines.append("### Class Isolation Scores (0=always co-occurs, 1=always alone)")
            isolation = co_occ['isolation_scores']
            sorted_isolation = sorted(isolation.items(), key=lambda x: x[1], reverse=True)
    ...
    def run_complete_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'summary': {
                'total_objects': self.analysis_results['basic_statistics']['total_objects'],
                'num_classes': len(self.classes),
                'splits': self.splits,
                'most_frequent_class': self.analysis_results['basic_statistics']['imbalance_metrics']['most_frequent_class'],
                'imbalance_ratio': self.analysis_results['basic_statistics']['imbalance_metrics']['imbalance_ratio']
            }
        }
