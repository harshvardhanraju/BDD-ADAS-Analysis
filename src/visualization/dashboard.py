...
def create_class_distribution_plots(data):
    ...
    with col1:
        ...
        gini = np.sum(np.abs(counts[:, None] - counts[None, :])) / (
            2 * len(counts) ** 2 * mean_count
        )
...
def create_spatial_analysis_plots(data):
    ...
    if len(bbox_data) > 5000:
        ...
        st.info(
            f"Showing sample of 5,000 objects from {len(bbox_data):,} total objects"
        )
    else:
    ...
    with col1:
        ...
        fig_width = px.histogram(
            bbox_data, x="bbox_width", title="Bounding Box Width Distribution", nbins=50
        )
...
def create_class_specific_analysis(data):
    ...
    if selected_class:
        ...
        bbox_class_data = class_data.dropna(
            subset=["bbox_x1", "bbox_y1", "bbox_x2", "bbox_y2"]
        )

        if len(bbox_class_data) > 0:
            bbox_class_data = bbox_class_data.copy()
            bbox_class_data["norm_center_x"] = (
                (bbox_class_data["bbox_x1"] + bbox_class_data["bbox_x2"]) / 2 / 1280
            )
            bbox_class_data["norm_center_y"] = (
                (bbox_class_data["bbox_y1"] + bbox_class_data["bbox_y2"]) / 2 / 720
            )
            ...
            with col1:
                ...
                fig_class_heat = px.density_heatmap(
                    ...
                    labels={
                        "norm_center_x": "Normalized X",
                        "norm_center_y": "Normalized Y",
                    },
                )
...
def create_interactive_filters(data):
    ...
    if "category" in data.columns:
        ...
        selected_classes = st.sidebar.multiselect(
            "Filter by Classes:", classes, default=["All"]
        )
...
def main():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">🚗 BDD100K Dataset Explorer</h1>',
        unsafe_allow_html=True,
    )
    st.markdown("---")
    ...
    **Filtered Dataset Info:**
    ...
    with tab1:
        ...
        if filtered_data["category"].notna().sum() > 0:
            ...
            with col1:
                st.info(
                    f"🏆 **Most Common Class**: {most_common_class} ({class_count:,} objects, {class_pct:.1f}%)"
                )
            ...
            with col2:
                ...
                imbalance_ratio = (
                    top_classes.iloc[0] / top_classes.iloc[-1]
                ...
                if imbalance_ratio > 10:
                    st.warning(
                        f"⚠️ **High Class Imbalance**: {imbalance_ratio:.1f}:1 ratio"
                    )
                elif imbalance_ratio > 5:
                    st.info(
                        f"ℹ️ **Moderate Class Imbalance**: {imbalance_ratio:.1f}:1 ratio"
                    )
                else:
                    st.success(
                        f"✅ **Well Balanced Classes**: {imbalance_ratio:.1f}:1 ratio"
                    )
